...
Key rules:
- Preserve `###` grading markers exactly where possible — lines ending with `###` and cells starting with `###` are used by the autograder. Keep them in both the assignment skeleton and the solution.
- **All top-level names must be unique across cells**, otherwise marimo raises `MultipleDefinitionError` at app load. If the Jupyter source defines `ode` (or any name) in two cells, rename one at the def site (e.g. `def ode_crossfeed(...)`). An `ode_crossfeed = ode` alias does NOT help — marimo's analyzer flags `def ode` as a top-level binding regardless of return. For variables only used within one cell (e.g. tuple unpacking like `δ1, δ2, β, ρ = ...`), prefix with `_` to make them cell-private.
- **Some `###` lines may need to be modified** to apply these renames/prefixes. When that happens, the autograder tests need a matching update (e.g. test `ode_crossfeed` instead of `ode`, accept `_δ1` in unpacking). Migration trades the inviolability of `###` lines for marimo's uniqueness rule.
